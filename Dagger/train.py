...
def sample(theta, env, N):
    ...
    for n in range(N):
        ...
        for t in range(200):
            ...
            probabilities = utils.compute_action_distribution(theta, phis)
            action = np.random.choice(np.arange(env.action_space.n), p=probabilities.flatten())
...
def train(N, T, delta, lamb=1e-3):
    ...
    for t in range(T):
        ...
        fisher_matrix = utils.compute_fisher_matrix(trajectories_grads, lamb)
        value_gradient = utils.compute_value_gradient(trajectories_grads, trajectories_rewards)
        eta = utils.compute_eta(delta, fisher_matrix, value_gradient)
        
        theta += eta * np.linalg.solve(fisher_matrix, value_gradient)
        avg_reward = np.mean([np.sum(rewards) for rewards in trajectories_rewards])
