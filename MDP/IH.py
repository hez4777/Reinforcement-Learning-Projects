...
class InfiniteHorizon:
    ...
    def computeQfromV(self, V):
        ...
        for s in range(self.nStates):
            for a in range(self.nActions):
                Q[s, a] = self.R[a, s] + self.discount * np.sum(self.P[a, s, :] * V)
    ...
    def valueIterationStep(self, Q):
        ...
        for a in range(self.nActions):
            for s in range(self.nStates):
                expect = np.sum(self.P[a, s, :] * Q_max)
                Q_next[s, a] = self.R[a, s] + self.discount * expect
    ...
    def approxPolicyEvaluation(self, pi, tolerance=0.01):
        ...
        while epsilon > tolerance:
            V_next = np.zeros(self.nStates)
            for s in range(self.nStates):
                a = pi[s]
                V_next[s] = self.R[a, s] + self.discount * np.sum(self.P[a, s, :] * V)
    ...
    def policyIterationStep(self, pi, exact):
        ...
        if exact:
            ...
            V, it_num, epsi = self.approxPolicyEvaluation(pi, tolerance=0.01)
    
        Q = self.computeQfromV(V)
        pi_next = np.argmax(Q, axis=1)
    ...
    def policyIteration(self, initial_pi, exact):
        ...
        while True:
            ...
            if exact:
                ...
                V, iter_num, epsi = self.approxPolicyEvaluation(pi, tolerance=0.01)
